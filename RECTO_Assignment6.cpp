...
Section: BSIT-1-YB-1
Activity Name: Machine Problem 6: File Input and Output
File Name: RECTO_Assignment6
*/
...
void studentdeets(struct Student *student) {
    printf("\nEnter number of courses for %s: ", student->name);
...
int main() {
    ...
    fprintf(summaryFile, "\nSUMMARY:\n");
    for (int i = 0; i < numStudents; i++) {
        stdsummary(&students[i], summaryFile);
    }

   
    fclose(summaryFile);

    printf("Summary saved to summary.txt\n");
